	...
	<body>
		<?php 
			...
			if(isset($_GET['delete']))
			{
				...
				$reset_inventory = mysqli_query($mysqli, "UPDATE inventory SET Mark = 4 WHERE ID = '$inventory'");
			...
			<div class="row">
							...
							<div class="form_content_view">
								<form class="form-horizontal" method=post action="<?php echo $_SERVER['PHP_SELF'];?>">
									<div class="form-group">
										<div class="col-sm-4">
											<label for="reference" class="control-label">Receipt</label>
										...
										<div class="col-sm-8">
											...
											<?php
												...
												for($i=0; $i<mysqli_num_rows($clients) and mysqli_num_rows($clients)>0; $i++)
												{
													...
													if($row[0] == $source) 
													{
														echo "<option value='".$row[0]."' SELECTED>".sprintf('%05d', $row[0])." ".ucwords(strtolower($row[1]))."</option>";
														$client_name = $row[1];
													}
				...
				<div class="selecttable" style="width:98%;margin-left:1%">
					...
					<?php
						...
						echo '<th style="width:13%;text-align:center">Actions</th>';
						echo '</tr></thead><tbody>';
					...
					<tr>
						...
						<td colspan=5>
							...
							<?php
								$available = mysqli_query($mysqli, "SELECT `ID`, `Name`, `Category`, `Weight` FROM inventory WHERE `Mark` = 4");
								for($i=0; $i<mysqli_num_rows($available) and mysqli_num_rows($available)>0; $i++)
								{
									$available->data_seek($i);
									$row = $available->fetch_row();
									
									$borrowed = mysqli_query($mysqli, "SELECT `Destination` FROM `transaction`, `particular` WHERE `Transaction` = `transaction`.`ID` AND `Inventory` = '$row[0]' AND `particular`.`Mark` = 1 ORDER BY `transaction`.`ID` DESC LIMIT 1");
									$borrowed = mysqli_fetch_row($borrowed);
									
									if($borrowed[0] == $source)
					...
					<?php
						...
						{	
							...
							//Delete
							echo '<td><a href="inventory_edit.php?inventory='.$row[0].'&sid='.$sid.'"><i class="fa fa-edit" aria-hidden="true"></i> Edit</a>&nbsp|&nbsp<a href="t_return.php?delete='.$row[0].'">Delete</a></td>';
				...
				<div class="selecttable" style="width:100%;margin-left:1%;margin-right:2%">
			        	<table class="table table-bordered table-stripped" style="font-size:12px;width:100%">
					<?php
					...
					<?php
						...
						{	
							...
							//Delete
							echo '<td><a href="t_return.php?pdelete='.$row[0].'">Delete</a></td>';
				...
				<div class="selecttable" style="width:98%;margin-left:1%">
			        	<table class="table" style="font-size:12px;width:100%">
			        		<tr style="text-align:center;">
			        			...
			        			<td style="width:8%"><a class='btn view_button' href='t_return.php?save=true' role='button'>Save</a></td>
			        			<td style="width:8%"><a class='btn view_button' href='t_return.php?cancel=true' role='button'>Cancel</a></td>
			        			<?php if($reference != "") { ?> <td style="width:8%"><a class='btn view_button' href='t_return.php?finalize=true' role='button'>Finalize</a></td> <?php } ?>
			        		</tr>
			        	</table>
			        </div>
			</div>
